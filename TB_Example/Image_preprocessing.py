# -*- coding: utf-8 -*-
"""
Created on Tue Jul  6 15:30:38 2021
...
#Plot Example
i = 1
im = cv2.imread('D:/File_X/Graduate/TB/MergeTB/TB (1).png')
...
#Transfer file path
original_dir = 'D:/File_X/Graduate/TB/Modeling'
if not os.path.isdir(original_dir): os.mkdir(original_dir)
train_dir = 'D:/File_X/Graduate/TB/Modeling/train'
if not os.path.isdir(train_dir): os.mkdir(train_dir)
test_dir = 'D:/File_X/Graduate/TB/Modeling/test'
if not os.path.isdir(test_dir): os.mkdir(test_dir)

train_normal_dir = train_dir + '/train_normal'
if not os.path.isdir(train_normal_dir): os.mkdir(train_normal_dir)   
train_tb_dir = train_dir + '/train_tb'
if not os.path.isdir(train_tb_dir): os.mkdir(train_tb_dir)
     
test_normal_dir = test_dir + '/test_normal'
if not os.path.isdir(test_normal_dir): os.mkdir(test_normal_dir)
test_tb_dir = test_dir + '/test_tb'
...
for i in range(len(test_df)):
    ...
    if x_test[i][27]  == 'N':  #Normal, TB
        ...
        new_filename = os.path.join(test_normal_dir, x_test[i][34:])
        status = cv2.imwrite(new_filename, img2)
        if status == False:
        ...
        img = cv2.imread(x_test[i]) 
        img2 = IF.crop_up_down_equalizeHist(img)
        new_filename = os.path.join(test_tb_dir, x_test[i][30:])
        status = cv2.imwrite(new_filename, img2)
